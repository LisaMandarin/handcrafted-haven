...
async function listArtisansByCategories(categoryId) {
...
export async function GET() {
    try {
        ...
        const categoryResults = await Promise.all([
            Object.entries(categories).map(async ([key, id]) => {
                try {
                    const data = await listArtisansByCategories(id)
                    return {[key]: data}
                } catch (error) {
                    console.error(`Error fetching ${key} artisans: `, error)
                    return {[key]: "Error fetching data"}
                }
            })
        ])

        const artisansData = categoryResults.reduce((acc, obj) => {
            return {...acc, obj}
        }, {})
